...
describe('Cron stuff', () => {
  ...
  it('test_generateAllSeries', async () => {
    expect(await queries.getleagueSeries()).toStrictEqual([]);
    ...
    expect((await queries.getleagueSeries()).length).toBe(6);
  });
